...
async def run_schema():
    ...
    migration_v21_path = os.path.join(os.path.dirname(__file__), "migration_v21.sql")
    if os.path.exists(migration_v21_path):
        async with get_conn() as conn:
            with open(migration_v21_path, "r") as f:
                await conn.execute(f.read())
        logger.info("Migration V21 applied successfully")

    migration_v22_path = os.path.join(os.path.dirname(__file__), "migration_v22.sql")
    if os.path.exists(migration_v22_path):
        async with get_conn() as conn:
            with open(migration_v22_path, "r") as f:
                await conn.execute(f.read())
        logger.info("Migration V22 applied successfully")

    migration_v23_path = os.path.join(os.path.dirname(__file__), "migration_v23.sql")
    if os.path.exists(migration_v23_path):
        async with get_conn() as conn:
            with open(migration_v23_path, "r") as f:
                await conn.execute(f.read())
